...
def send_whatsapp_message(phone, name, enrollment_no, marks):
    ...
    phone_no = f"+91{phone}"  # Format the phone number for WhatsApp

    # now = datetime.datetime.now()
    # hour = now.hour
    # minute = (now.minute + 2) % 60  # Add 2 minutes but wrap around if it goes beyond 59
    # if now.minute + 2 >= 60:
    #     hour = (hour + 1) % 24 
    now = datetime.datetime.now()
    hour = now.hour
    ...
    try:
        kit.sendwhatmsg(phone_no, message, hour, minute, wait_time=30, tab_close=True)
        print(f"WhatsApp message scheduled to be sent to {name} at {phone_no}")
    except Exception as e:
        print(f"Failed to send WhatsApp message to {name} at {phone_no}: {e}")
