...
def create_excel_files():
    ...
    marks_data = {
        ...
        'Enrollment No': ['0801CS221017', '0801CS221016', '0801CS221018', '0801CS221019', '0801CS221020'],
        'Marks': [85, 90, 78, 88, 92]
    }
    
    # Sample data for students' phone numbers
    phone_data = {
        'Student Name': ['Alice', 'Bob', 'Charlie', 'Eve', 'Frank'],
        'Enrollment No': ['0801CS221017', '0801CS221016', '0801CS221018', '0801CS221020', '0801CS221022'],
        'Phone No': ['7879397605', '9340405433', '9993361269', '7879397605', 'invalid_number']
    }
    ...
    # Save to Excel files
    marks_df.to_excel('marksFINAL.xlsx', index=False)
    phone_df.to_excel('phonesFINAL.xlsx', index=False)
    
    return marks_df, phone_df

def find_invalid_students(marks_df, phone_df):
    # Merge DataFrames on 'Student Name' and 'Enrollment No'
    ...
    result_df = invalid_students[['Student Name', 'Enrollment No', 'Phone No', '_merge']]
...
def send_whatsapp_messages(marks_df, phone_df):
    ...
    merged_df = pd.merge(marks_df, phone_df, on=['Student Name', 'Enrollment No'], how='inner')

    for index, row in merged_df.iterrows():
        message = f"Hello {row['Student Name']}, your marks are {row['Marks']}."
        phone_number = row['Phone No']
        try:
            kit.sendwhatmsg_instantly(f"+91{phone_number}", message)
            print(f"Message sent to {row['Student Name']} at {phone_number}.")
        except Exception as e:
            print(f"Failed to send message to {row['Student Name']} at {phone_number}: {str(e)}")
